...
public class BlobDetectorToCubeInLocalSpace : MonoBehaviour
{
    ...
    public bool m_useInitialEnvironmentFilter = true;
    /// <summary>
    ...
    void Update()
    {
        ...
        if (m_depthPosition == null || m_depthPosition.Length != depthImageWidth * depthImageHeight)
        {
            m_depthPosition = new Vector3[depthImageWidth * depthImageHeight];
            kinectManager.GetSensorData().depth2SpaceCoords = m_depthPosition;
        }
        // calculate the foreground rectangle
    ...
    public KinectUShortDepthRef m_depthRef;
    public KinectUshortDepthRefEvent m_createdRefDepth;
    public KinectUShortDepthRef m_depthRefFiltered;
    public KinectUshortDepthRefEvent m_createdRefDepthFilter;

    public Vector3[] m_depthPosition;
    private void DetectBlobsInRawDepth()
    {
        ushort[] rawDepth = kinectManager ? kinectManager.GetRawDepthMap() : null;
        if (m_depthRef == null || m_depthRef.m_arrayRef.Length != rawDepth.Length)
        {
            m_depthRef = new KinectUShortDepthRef(kinectManager.GetDepthImageWidth(), kinectManager.GetDepthImageHeight(), rawDepth.Length);
            m_createdRefDepth.Invoke(m_depthRef);
        }

        if (m_depthRefFiltered == null || m_depthRefFiltered.m_arrayRef.Length != rawDepth.Length)
        {
            m_depthRefFiltered = new KinectUShortDepthRef(kinectManager.GetDepthImageWidth(), kinectManager.GetDepthImageHeight(), rawDepth.Length);
            m_createdRefDepthFilter.Invoke(m_depthRefFiltered);
        }
        



        if (m_requestSaveDepthBackground || m_depthBackground == null)
        {
            m_requestSaveDepthBackground = false;
            SaveDepthToRemove(rawDepth);
        }
        ...
        for (int y = 0, di = 0; y < depthImageHeight; y += xyIncrement)
        {
            ...
            for (int x = 0; x < depthImageWidth; x += xyIncrement, di += xyIncrement)
            {
                ...
                m_depthRef.m_arrayRef[di] = depth;
                depth = (depth >= minDistanceMm && depth <= maxDistanceMm) ? depth : (ushort)0;

                if (m_useInitialEnvironmentFilter && depth >= (m_depthBackground[di] - m_rangeToIgnore))
                    depth = (ushort)0;
                m_depthRefFiltered.m_arrayRef[di] = depth;
    ...
    [ContextMenu("Save depth to remove")]
    public void SaveDepthToRemoveNextFrame()
    {
        m_requestSaveDepthBackground = true;
    }
    public void SaveDepthToRemove(in ushort[] depthArray)
    {
        if (m_depthBackground == null || m_depthBackground.Length != depthArray.Length)
        {
            m_depthBackground = new ushort[depthArray.Length];
        }
        for (int i = 0; i < depthArray.Length; i++)
        {
            m_depthBackground[i] = depthArray[i];
        }


    }
    bool m_requestSaveDepthBackground;
    ushort[] m_depthBackground;
    public ushort m_rangeToIgnore = 10;
