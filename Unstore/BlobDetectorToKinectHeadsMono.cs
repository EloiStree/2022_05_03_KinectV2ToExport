...
public class BlobDetectorToKinectHeadsMono : MonoBehaviour
{
    ...
    public KinectUShortDepthRef m_depthRefFilter;
    ...
    private void DetectBlobsInRawDepth()
    {
        ...
        if (m_depthRefFilter == null || m_depthRefFilter.m_arrayRef.Length != rawDepth.Length)
        {
            m_depthRefFilter = new KinectUShortDepthRef(kinectManager.GetDepthImageWidth(), kinectManager.GetDepthImageHeight(), rawDepth.Length);
            m_createdRefDepthFilter.Invoke(m_depthRefFilter);
        }
        ...
        for (int y = 0, di = 0; y < depthImageHeight; y += xyIncrement)
        {
            ...
            for (int x = 0; x < depthImageWidth; x += xyIncrement, di += xyIncrement)
            {
                ...
                if(depth >= (m_depthBackground[di] - m_rangeToIgnore))
                    depth =(ushort)0 ;
                m_depthRefFilter.m_arrayRef[di] = depth;
    ...
    public Transform m_kinectBlobRoot;
    public List<GameObject> m_createBlobPosition= new List<GameObject>();
